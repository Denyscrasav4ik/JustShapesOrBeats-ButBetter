...
var bullets: Array = []
var shapes: Array = []
var dance_append_calls: Callable
...
func create_shape(dict: Dictionary) -> Array:
	...
	PhysicsServer2D.shape_set_data(shape, dict.data)
	return [shape]
...
func create_bullet(type: int, is_dance_floor: bool, pos: Vector2, speed: Vector2, size: float, extras: Dictionary = {}) -> Bullet:
	...
	bullet.speed = speed
	bullet.area = PhysicsServer2D.area_create()
	bullet.type = type
	...
	PhysicsServer2D.area_set_space(bullet.area, get_world_2d().space)

	for i in shapes[type]:
		PhysicsServer2D.area_add_shape(bullet.area, i)
	...
	PhysicsServer2D.area_set_monitorable(bullet.area, true)
	PhysicsServer2D.area_set_collision_mask(bullet.area, 0)
...
func spawn_bullet(bullet: Bullet):
	...
	tween.tween_property(bullet, "animation_appear", 1.0, bullet.animation_time)
	tween.tween_callback(bullet.set_animation_type.bind(Bullet.ANIMATION_NONE))
	tween.tween_callback(set_all_shapes_disabled.bind(bullet, false))
...
func end_bullet(bullet: Bullet):
	...
	var tween = create_tween()
	tween.tween_property(bullet, "animation_disappear", 1.0, bullet.animation_time)
	tween.tween_callback(find_and_remove_bullet.bind(bullet))
...
func set_all_shapes_disabled(bullet: Bullet, disable: bool):
	...
	for i in shapes[bullet.type].size():
		PhysicsServer2D.area_set_shape_disabled(bullet.area, i, disable)
...
func _draw():
	for i in bullets:
		...
		if i.animation_type == Bullet.ANIMATION_APPEAR:
			xform.x *= appear_curve.sample_baked(i.animation_appear)
			xform.y *= appear_curve.sample_baked(i.animation_appear)
		if i.animation_type == Bullet.ANIMATION_DISAPPEAR:
			xform.x *= disappear_curve.sample_baked(i.animation_disappear)
			xform.y *= disappear_curve.sample_baked(i.animation_disappear)
		...
		if i.animation_type == Bullet.ANIMATION_APPEAR and not GameSettings.photosens_mode:
			color = APPEAR_GRAD.sample(i.animation_appear)
...
func draw_bullet(bullet: Bullet, color: Color, mode: int):
	match bullet.type:
		...
		BULLET_SQUARE_STRIPES:
			...
			for i in int(bullet.drawing_extra):
				draw_colored_polygon(
						Transform2D(
								Vector2(1 - i * difference, 0),
								Vector2(0, 1 - i * difference),
								Vector2.ZERO
						) * TS.POOL_VEC2_HALF_CENTERED_SQUARE,
...
func remove_bullet(pos: int, remove_on_list: bool = true):
	PhysicsServer2D.free_rid(bullets[pos].area)
