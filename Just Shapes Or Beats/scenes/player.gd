...
@onready var texture: Texture2D = TS.PLAYER_TEXTURES[player_index][0]
@onready var inside_texture: Texture2D = TS.PLAYER_TEXTURES[player_index][1]
...
func _ready():
	...
	DashCircle.set_as_top_level(true)
	...
	tween_property(
			HelpDrift,
			"position",
			HelpDrift.position + Vector2(0, - DRIFT_TEXT_OFFSET),
			DRIFT_TEXT_HALF_TIME
	)
	tween.tween_property(HelpDrift, "modulate", Color.WHITE, DRIFT_TEXT_HALF_TIME)
	tween.parallel().tween_property(HelpDrift, "position", HelpDrift.position, DRIFT_TEXT_HALF_TIME)
	update_hp()
...
func _physics_process(delta):
	...
	if is_dash_pressed() and not is_dashing() and not stunned:
		...
		var __ = dash_timer.connect("timeout", Callable(self, "stop_dashing").bind(dash_timer))

		var target: GPUParticles2D = DashBurst if not DashBurst.emitting else DashBurst2
	...
	if dash_timer and dash_timer.time_left >= 0:
		...
		extra_speed *= lerp(1.0, DASH_MULTI as float, DASH_CURVE.sample_baked(dash_point))

		if dashed_still:
			dir = Vector2(0.85, 0.0)
	...
	if is_moving:
		var new_target = TS.vec2_home(Vector2.ZERO, dir, Sprites.transform.x, 1.0)
		if not rotation_tween or not rotation_tween.is_valid() or new_target != target_rotation:
			target_rotation = new_target

			if rotation_tween and rotation_tween.is_valid():
				rotation_tween.kill()

			Sprites.rotation = wrapf(Sprites.rotation, -PI, PI)
			rotation_tween = create_tween()

			var short_target = Sprites.rotation + angle_difference(Sprites.rotation, target_rotation)

			rotation_tween.tween_property(Sprites, "rotation", short_target, 0.35)\
		...
		if was_moving:
			...
			var short_target = Sprites.rotation + angle_difference(Sprites.rotation, 0.0)

			rotation_tween.tween_property(Sprites, "rotation", short_target, 0.45)\
				.set_trans(Tween.TRANS_BACK)\
				.set_ease(Tween.EASE_OUT)
		elif not rotation_tween or not rotation_tween.is_valid():
			Sprites.rotation = rotate_toward(Sprites.rotation, 0.0, ROTATE_SPEED * delta)

	was_moving = is_moving

	var lenght = dir.length()

	var target_scale_x = lerp(1.0, remap(
	...
	Sprites.scale.x = lerp(Sprites.scale.x, target_scale_x, delta * SQUISH_SPEED)
	Sprites.scale.y = 1.0 / Sprites.scale.x

	var offset_vec: Vector2 = Vector2(remap(
		dash_timer.time_left if dash_timer else 0.0,
		0,
		DASH_TIME,
		0,
	...
	BaseSprite.scale = Vector2.ONE * remap(
			WHITE_SCALE_CURVE.sample_baked(white_weight),
			1.0,
			0.0,
			DASH_WHITE_SCALE,
			1.0
...
func stop_dashing(timer_ref: SceneTreeTimer = null):
	if timer_ref != null and timer_ref != dash_timer:
		return
...
func get_hit():
	...
	if hp <= 0:
		...
		stunned = true
		BreakParti.restart()
		var __ = get_tree().create_timer(MERCY_INVI_TIME, false).connect("timeout", Callable(self, "end_mercy_invi"))
		var ___ = get_tree().create_timer(STUN_TIME, false).connect("timeout", Callable(self, "end_stun"))
...
func set_drift_stuff():
	RescueArea.set_collision_mask_value(1, drifting)
...
func rescue():
	...
	update_hp()
	var __ = get_tree().create_timer(MERCY_INVI_TIME, false).connect("timeout", Callable(self, "end_mercy_invi"))
	var ___ = get_tree().create_timer(MERCY_INVI_TIME, false).connect("timeout", Callable(self, "end_mercy_by_rescue"))
